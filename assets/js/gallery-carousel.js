...
(async function initCarousel() {
    ...
    function render(slideIndex) {
        ...
        // Construct URL safely with fallback
        let src;
        try {
            const baseURL = baseDir.startsWith('/') ? window.location.origin + baseDir : baseDir;
            const normalizedBase = baseURL.endsWith('/') ? baseURL : baseURL + '/';
            src = new URL(item.src, normalizedBase).toString();
        } catch (error) {
